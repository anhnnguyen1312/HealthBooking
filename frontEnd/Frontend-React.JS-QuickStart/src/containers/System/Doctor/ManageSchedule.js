...
import { saveScheduleDoctorServices } from "../../../services";
class ManageSchedule extends Component {
  ...
  handleChange = async (selectedDoctor) => {
    //   await this.props.getScheduleDoctorByDateRedux(
  ...
  handleSetDate = async (date) => {
    ...
    //   }
    console.log("date.setHours(0, 0, 0, 0)", date, date.setHours(0, 0, 0, 0));
  ...
  handleClickSaveSchedule = async () => {
    ...
    let formatedDDate =
      ...
      new Date(initDate).getTimezoneOffset() * 60000 +
      7 * 3600 * 1000;

    console.log("initDate", initDate);
    console.log("formatedDDate", formatedDDate);

    // let formatedDDate = moment(initDate).unix();
    // let formatedDDate = moment(initDate).format(dateFormat.SEND_TO_SERVER);
    //console.log(formatedDDate);
    // if (selectedDoctor || initDate) {
    if (!initDate) {
      toast.warning("Missing information range date");
    } else {
      if (listSchedule && listSchedule.length > 0) {
        let selectedTime = listSchedule.filter(
        ...
        let response = await this.props.saveScheduleDoctorRedux(arrResult);
    ...
    console.log("this.props.userInfo.id", this.props.userInfo);
  ...
  render() {
    ...
    let { languageRedux, scheduleRedux, userInfo } = this.props;
            ...
            {listSchedule.map((range) => {
              return (
                <div
                  key={range.id}
                  className={
                    range.isSelected ? "range-time selected" : "range-time"
                  }
                  onClick={() => {
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    saveScheduleDoctorRedux: (data) =>
      dispatch(actions.saveScheduleDoctor(data)),
